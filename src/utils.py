...
def smooth_ts(ts, framerate):
    # return median_filter(butter_lowpass_filter(ts, 6, framerate), 2)
...
def process_subject(subjectid, processed_npy_path="videos/np/", framerate = None, to_smooth = True):
    ...
    for i in range(res.shape[1]//3):
        ...
        res[:,3*i:(3*i+2)] = (res[:,3*i:(3*i+2)])/height
